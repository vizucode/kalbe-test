package employee

import (
	"context"
	"log"

	"api.kalbe.crm/apps/domain"
	"api.kalbe.crm/apps/models"
	"api.kalbe.crm/config/timestamp"
	"github.com/gofiber/fiber/v2"
	"golang.org/x/crypto/bcrypt"
)

func (s *employee) CreateEmployee(ctx context.Context, payload domain.Employee) (err error) {
	err = s.validator.StructCtx(ctx, &payload)
	if err != nil {
		log.Println(err)
		return fiber.NewError(fiber.StatusBadRequest, fiber.ErrBadRequest.Message)
	}

	bytePass, err := bcrypt.GenerateFromPassword([]byte(payload.Password), bcrypt.DefaultCost)
	if err != nil {
		log.Println(err)
		return fiber.NewError(fiber.StatusInternalServerError, fiber.ErrInternalServerError.Message)
	}

	employees, err := s.db.GetAllEmployee(ctx)
	if err != nil {
		log.Println(err)
		return err
	}

	err = s.db.CreateEmployee(ctx, models.Employee{
		EmployeeCode:  timestamp.GenerateCode(len(employees)),
		Password:      string(bytePass),
		EmployeeName:  payload.EmployeeName,
		DepartementId: payload.DepartementId,
		PositionId:    payload.PositionId,
		Superior:      payload.Superior,
	})
	if err != nil {
		log.Println(err)
		return err
	}

	return nil
}
